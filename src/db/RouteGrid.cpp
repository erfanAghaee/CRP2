...
void RouteGrid::init() {
    ...
    if(db::setting.debug){
        log() << "db::Setting::weightWirelength: " << db::Setting::weightWirelength << std::endl;
        log() << "unitWireCostRaw: " << unitWireCostRaw << std::endl;
    }
    ...
    for (int i = 0; i < layers.size(); ++i) {
        // log() << "unitShortVioCostRaw: " << unitShortVioCostRaw 
    ...
    //     log() << "unitShortVioCost#" << i << ": " << unitShortVioCost[i] << std::endl;

    unitShortVioCostDiscounted.resize(unitShortVioCost.size());
    setUnitVioCost();

    // log() << "unitShortVioCostDiscounted layers..." << std::endl;
    // for(int i = 0 ; i < unitShortVioCostDiscounted.size() ; i++)
    //     log() << "unitShortVioCostDiscounted#" << i << ": " << unitShortVioCostDiscounted[i] << std::endl;
...
void RouteGrid::clear() {
    // Fixed metal
    fixedMetals.clear();
    unitShortVioCost.clear();
    unitShortVioCostDiscounted.clear();  
...
void RouteGrid::setUnitVioCost(double discount) {
    ...
    if(debug){
        ...
        log() << "before update unitShortVioCostDiscounted: " << std::endl;
        for(auto vio_cost_discount : unitShortVioCostDiscounted){
            log() << "vio_cost_discount: " << vio_cost_discount << std::endl;
        }
    }

    if(debug)
        log() << "unitShortVioCost.size(): " <<  unitShortVioCost.size() << std::endl;
    for (int i = 0; i < unitShortVioCost.size(); ++i) {
    ...
    if(debug){
        log() << "after update unitShortVioCostDiscounted: " << std::endl;
        for(auto vio_cost_discount : unitShortVioCostDiscounted){
            log() << "vio_cost_discount: " << vio_cost_discount << std::endl;
        }
    }
