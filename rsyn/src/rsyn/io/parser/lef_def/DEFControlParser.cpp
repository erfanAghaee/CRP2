...
int defCompf(defrCallbackType_e c, defiComponent* co, defiUserData ud) {
	...
	// std::cout << "component " << co->name() << " " << defComp.clsName  << std::endl;
...
int defNetWires(defiNet* net, DefNetDscp& netDscp) {
	...
	if(debug){
		std::cout << "num wires: " << net->numWires() << std::endl;
	}
	...
	for (unsigned i = 0; static_cast<int>(i) < net->numWires(); ++i) {
		...
		for (unsigned j = 0; static_cast<int>(j) < wire->numPaths(); ++j) {
			...
			if(debug){
				std::cout << "wire->numPaths(): " << wire->numPaths() << std::endl;
			}
			...
			while (pathId != DEFIPATH_DONE) {
				switch (pathId) {
					case DEFIPATH_LAYER:
						if(debug)
							std::cout << "layer: " << path->getLayer() << std::endl;
						segmentDscp.clsLayerName = path->getLayer();
						break;
					case DEFIPATH_VIA:
						if(debug)
							std::cout << "via: " << path->getVia() << std::endl;
					...
					case DEFIPATH_POINT:
						...
						if(debug){
							std::cout << "x: " << point->clsPos.x/2000.0 
								      << ", y: " << point->clsPos.y/2000.0 << std::endl;
						}
					...
					case DEFIPATH_RECT:
						...
						if(debug){
							std::cout << "lx: "   << point->clsRect[LOWER][X]/2000.0
									  << ", ly: " << point->clsRect[LOWER][Y]/2000.0
									  << ", hx: " << point->clsRect[UPPER][X]/2000.0
									  << ", hy: " << point->clsRect[UPPER][Y]/2000.0 << std::endl;							
						}
						break;
...
int defSpecialNet(defrCallbackType_e c, defiNet* net, void* ud) {
	...
	if(debug){
		std::cout << "special net: " << net->name() << std::endl;
	}
...
int defGCellGrid(defrCallbackType_e typ, defiGcellGrid * gcell, defiUserData ud) {
	...
	// std::cout << "gcell->x(): " << gcell->x() << std::endl;
	return 0;
} // end method 
